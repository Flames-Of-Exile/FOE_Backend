...
class AuthTests(BasicTests):
    ...
    def test_register_success(self):
        user = User('new', sha256_crypt.encrypt('1qaz!QAZ'), 'new@email.com', self.DEFAULT_GUILD.id, User.Role.GUEST)
        user.id = 2
    ...
    def test_register_fail_empty_password(self):
        response = self.register('new', '', 'new@email.com', self.DEFAULT_GUILD.id)
        self.assertEqual(response.status_code, 400)
        self.assertNotIn('Set-Cookie', response.headers)
    ...
    def test_password_complexity_length_fail(self):
        response = self.register('new', '1qaz!QA', 'new@email.com', self.DEFAULT_GUILD.id)
        self.assertEqual(response.status_code, 400)
    ...
    def test_password_complexity_digit_fail(self):
        response = self.register('new', 'qqaz!QAZ', 'new@email.com', self.DEFAULT_GUILD.id)
        self.assertEqual(response.status_code, 400)
    ...
    def test_password_complexity_symbol_fail(self):
        response = self.register('new', '1qaz1QAZ', 'new@email.com', self.DEFAULT_GUILD.id)
        self.assertEqual(response.status_code, 400)
    ...
    def test_password_complexity_multi_fail(self):
        response = self.register('new', '1qaz', 'new@email.com', self.DEFAULT_GUILD.id)
        self.assertEqual(response.status_code, 400)
